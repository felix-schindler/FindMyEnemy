...
export default class UserController extends AuthController {
	...
	override async register(
		...
		// @ts-ignore - Ignore ID since it's generated by DB
		const user: User = {
			username,
			email,
		...
		const id = await db.create<User>("users", user);
		user.id = id;
	...
	async getList(c: Context<Env, "/users">): Promise<Response> {
		...
		const users = (await db.query<ClientUser[]>(
	...
	async get(c: Context<Env, "/users/:id">): Promise<Response> {
		...
		const user = (await db.select<User, string>(`users:${id}`))[0];
